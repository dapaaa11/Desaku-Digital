...
- [15 May 2026] End-to-End Testing Menyeluruh (Backend, CMS Admin, Mobile Flutter): Memperbaiki 8 bug kritis. Frontend build: success. Backend build: success. Mobile build: success. Semua fitur stabil.
- [15 May 2026] Finalisasi Production Readiness (v1.0): Refactor URL backend hardcoded (localhost/10.0.2.2) menjadi environment configurable via NEXT_PUBLIC_API_URL (Next.js) dan --dart-define=API_URL (Flutter). Melakukan pass pada strict eslint dan types rules di Next.js dan NestJS. Melakukan build untuk NestJS production (dist), Next.js production (.next), dan Flutter build apk (--release) sebesar 48.1MB. Project siap deploy ke server production. Status Project: Stabil & Selesai.